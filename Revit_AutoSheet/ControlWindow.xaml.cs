...
namespace Revit_CreateRoomElevation1
{
    ...
    public partial class ControlWindow : Window
    {
        ...
        public ControlWindow(UIApplication uiapp, Room selRoom)
        {
            InitializeComponent();
            _uiapp = uiapp;
            _uidoc = _uiapp.ActiveUIDocument;
            _doc = _uidoc.Document;
            _selection = _uidoc.Selection;
            Selection Ids = _uidoc.Selection;
            foreach (ElementId eleId in Ids.GetElementIds())
            {
                _selRoom = _doc.GetElement(eleId) as Room;
                break;
            }
            if (_selRoom == null)
                labRoomId.Content = "N/A";
            else
                labRoomId.Content = _selRoom.Id.ToString();
        }
        ...
        private void BtnRun_Click(object sender, RoutedEventArgs e)
        {
            ...
            double FloorThickness = 0;//地面楼板的厚度 用于将里面的底边框上抬到楼板上
            ...
            if (_selRoom == null)
            {
                ...
                this.Hide();
                _selRoom = _doc.GetElement(_selection.PickObject(ObjectType.Element, "Pleace selecet a room")) as Room;
                if (_selRoom == null)
                {
            ...
            try
            {
                Parameter SoANuber = _selRoom.LookupParameter("SoA Number");
            ...
            using (Transaction tran = new Transaction(_doc))
            {
                foreach (ViewFamilyType viewFamilyType in collector)
                {
                        ...
                        || viewFamilyType.ViewFamily == ViewFamily.CeilingPlan)
                    {
                        ...
                        view = ViewPlan.Create(_doc, viewFamilyType.Id, _selRoom.LevelId);

                        //TaskDialog.Show("CreatLevelView", "A new level's view has been Created");

                        view.Name = _selRoom.Name;//生成平面的名称
                        ...
                        view.get_Parameter(BuiltInParameter.VIEW_DESCRIPTION).Set(_selRoom.Name);

                        if (viewFamilyType.ViewFamily == ViewFamily.CeilingPlan)
                        {
                            PlanViewRange range = view.GetViewRange();
                            range.SetLevelId(PlanViewPlane.TopClipPlane, _selRoom.UpperLimit.Id);
                            range.SetLevelId(PlanViewPlane.ViewDepthPlane, _selRoom.UpperLimit.Id);
                            range.SetLevelId(PlanViewPlane.CutPlane, _selRoom.LevelId);
                            range.SetLevelId(PlanViewPlane.BottomClipPlane, _selRoom.LevelId);
                            ...
                            viewName = "RCP ";
                            view.get_Parameter(BuiltInParameter.VIEW_DESCRIPTION).Set(_selRoom.Name + " - RCP");

                        }
                        viewName += _SoANumber + "_" + _selRoom.Level.Name;
        ...
        public void CreateElevations(double elevationOffset,double FloorThickness)
        {
            ...
            {
                //房间的"X"的交点
                LocationPoint pt = _selRoom.Location as LocationPoint;

                tran.Start("newElvation");
                ElevationMarker marker = ElevationMarker.CreateElevationMarker(_doc, viewTypeId, pt.Point, 50);
                for (; i < 4; i++)
                {
                    ViewSection sv = marker.CreateElevation(_doc, _doc.ActiveView.Id, i);
        ...
        //修剪边框      暂时不用 
        public CurveLoop TrimCurveLoop(CurveLoop loop, View view)
        {
            foreach (Curve curve0 in loop)
            {
                XYZ point0 = curve0.GetEndPoint(0);
                XYZ point1 = null;
                foreach (Curve curve1 in loop)
                {
                    point1 = curve1.GetEndPoint(1);
                    if (curve0 == curve1 || point0.DistanceTo(point1) > 10)
                        break;
                }
            }
            return loop;
        ...
        public View3D Create3DView()
        {
            using (Transaction tran = new Transaction(_doc))
            {
                var collector = new FilteredElementCollector(_doc).OfClass(typeof(ViewFamilyType));
                var viewFamilyTypes = from elem in collector
                                      let type = elem as ViewFamilyType
                ...
                View3D view = View3D.CreateIsometric(_doc, viewFamilyTypes.First().Id);

                //设定一个新的截面框
                BoundingBoxXYZ boxXyz = new BoundingBoxXYZ();
                XYZ max = _selRoom.get_BoundingBox(view).Max;
                XYZ min = _selRoom.get_BoundingBox(view).Min;
                ...
                name += _SoANumber;
                view.ViewName = name;
                var par = view.get_Parameter(BuiltInParameter.VIEW_DESCRIPTION).Set(_selRoom.Name + " - ISOMETRIC");
                tran.Commit();
        ...
        public void ChangeViewFitRoom(ViewPlan view,Transaction tran,double viewOffseet)
        {
            ...
            IList<IList<BoundarySegment>> segments = _selRoom.GetBoundarySegments(opt);
        ...
        public  void CreateSheetView(View3D view3D)
        {
            ...
            if (fs != null)
            {
                using (Transaction t = new Transaction(_doc, "Create a new ViewSheet"))
                {
                    ...
                    {
                        // Create a sheet view
                        ViewSheet viewSheet = ViewSheet.Create(_doc, fs.Id);
                        if (null == viewSheet)
                        {
                        ...
                        Viewport.Create(_doc, viewSheet.Id, view3D.Id, new XYZ(location.U, location.V, 0));
